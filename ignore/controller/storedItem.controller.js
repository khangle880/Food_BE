const httpStatus = require('http-status');
const pick = require('../utils/pick');
const ApiError = require('../utils/ApiError');
const catchAsync = require('../utils/catchAsync');
const { storedItemService } = require('../services');

const create = catchAsync(async (req, res) => {
  const item = await storedItemService.create(req.body);
  res.status(httpStatus.CREATED).send(item);
});

const getItems = catchAsync(async (req, res) => {
  let filter = pick(req.query, ['modelId']);
  const { data } = pick(req.body, ['data']);
  if (data) {
    const prefix = 'data.';
    const flatData = Object.assign({}, ...Object.keys(data).map((key) => ({ [prefix + key]: data[key] })));
    filter = { ...filter, ...flatData };
  }

  const options = pick(req.query, ['sortBy', 'limit', 'page']);
  const result = await storedItemService.query(filter, options);
  res.send(result);
});

const getById = catchAsync(async (req, res) => {
  const item = await storedItemService.getById(req.params.id);
  if (!item) {
    throw new ApiError(httpStatus.NOT_FOUND, 'Stored item not found');
  }
  res.send(item);
});

const updateById = catchAsync(async (req, res) => {
  const item = await storedItemService.updateById(req.params.id, req.body);
  res.send(item);
});

const deleteById = catchAsync(async (req, res) => {
  await storedItemService.deleteById(req.params.id);
  res.status(httpStatus.OK).send('Delete Successfully');
});

module.exports = {
  create,
  getItems,
  getById,
  updateById,
  deleteById,
};
